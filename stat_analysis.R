...
#input file path 
input_R_v2 <- read_csv("/Users/zebulon/Documents/UniDocs/Year4/R250/Non-standard-NLP/Project/new_corpus.csv", 
                       col_types = cols(sent_type = col_factor(levels = c("non-code-switch", "code-switch")),
                                        ...
                                        # Zeb's calculated surprisal and mutual information
                                        zeb_surprisal = col_double(),
                                        zeb_mutual = col_double()
                       )
                       , trim_ws = FALSE)
...
# Zeb's surprisal and mutual information
plot(input_R_v2$sent_type, input_R_v2$zeb_surprisal, xlab="Sentence Type", ylab="Surprisal at CS-Point")
plot(input_R_v2$sent_type, input_R_v2$zeb_mutual, xlab="Sentence Type", ylab="Mutual Information at CS-Point")
...
myvars <- c("surprisal_first_cs_word_trans", "surprisal_of_previous_word", "deviation_from_lastword_surprisal")
new_view<-input_R_v2[myvars]
View(new_view)
...
# Zeb surprisal and mutual information
input_R_v2$zeb_surprisal <- scale(input_R_v2$zeb_surprisal, center=TRUE, scale=TRUE)
input_R_v2$zeb_mutual <- scale(input_R_v2$zeb_mutual, center=TRUE, scale=TRUE)
...
# Zeb surprisal and mutual information, and combinations with previous surprisal
m_surp_zeb <- glm(sent_type ~ zeb_surprisal, data=input_R_v2, family="binomial")
summary(m_surp_zeb)
m_mutual_zeb <- glm(sent_type ~ zeb_mutual, data=input_R_v2, family="binomial")
summary(m_mutual_zeb)

m_surp2 <- glm(sent_type ~ surprisal_first_cs_word_trans + zeb_surprisal, data=input_R_v2, family="binomial")
summary(m_surp2)
m_surp_mut <- glm(sent_type ~ zeb_mutual + surprisal_first_cs_word_trans, data=input_R_v2, family="binomial")
summary(m_surp_mut)
m_surp2_mut <- glm(sent_type ~ zeb_mutual + zeb_surprisal, data=input_R_v2, family="binomial")
summary(m_surp2_mut)
...
### ZEB MUTUAL INFORMATION AND SURPRISAL
m_complete_zeb_surprisal <- glm(sent_type ~ zeb_surprisal+ postag_converted+length_first_cs_word_trans+translation_sentence_length+frequency_negative_ln_first_cs_word_trans, data=input_R_v2, family = "binomial")
summary(m_complete_zeb_surprisal)
bic(m_complete_zeb_surprisal)

m_complete_zeb_mutual <- glm(sent_type ~ zeb_mutual+ postag_converted+length_first_cs_word_trans+translation_sentence_length+frequency_negative_ln_first_cs_word_trans, data=input_R_v2, family = "binomial")
summary(m_complete_zeb_mutual)
bic(m_complete_zeb_mutual)

m_complete_zeb_both <- glm(sent_type ~ zeb_mutual+zeb_surprisal+ postag_converted+length_first_cs_word_trans+translation_sentence_length+frequency_negative_ln_first_cs_word_trans, data=input_R_v2, family = "binomial")
summary(m_complete_zeb_both)
bic(m_complete_zeb_both)
...
# ZEB SURPRISAL AND MUTUAL INFORMATION
cor(input_R_v2$zeb_surprisal, input_R_v2$zeb_mutual, method="spearman")
# -0.8388127
vif(m_complete_zeb_both)
...
# ZEB SURPRISAL AND MUTUAL INFORMATION
lrtest(m_control_bic1, m_complete_zeb_surprisal)
#Df  LogLik Df  Chisq Pr(>Chisq)    
#1   6 -1977.6                         
#2   7 -1957.9  1 39.474  3.324e-10 ***

lrtest(m_control_bic1, m_complete_zeb_mutual)
#Df  LogLik Df  Chisq Pr(>Chisq)    
#1   6 -1977.6                         
#2   7 -1942.5  1 70.207  < 2.2e-16 ***

# Both minus surprisal
lrtest(m_complete_zeb_mutual, m_complete_zeb_both)
#Df  LogLik Df  Chisq Pr(>Chisq)    
#1   7 -1942.5                         
#2   8 -1934.1  1 16.881   3.98e-05 ***

# Both minus mi
lrtest(m_complete_zeb_surprisal, m_complete_zeb_both)

# Both minus frequency
m_complete_zeb_both_nofreq <- glm(sent_type ~ zeb_surprisal + zeb_mutual +postag_converted+ length_first_cs_word_trans+translation_sentence_length, data=input_R_v2, family = "binomial")
lrtest(m_complete_zeb_both_nofreq, m_complete_zeb_both)

# Both minus word length
m_complete_zeb_both_nowordlen <- glm(sent_type ~ zeb_surprisal + zeb_mutual +postag_converted+ frequency_negative_ln_first_cs_word_trans +translation_sentence_length, data=input_R_v2, family = "binomial")
lrtest(m_complete_zeb_both_nowordlen, m_complete_zeb_both)

# Both minus sentence length
m_complete_zeb_both_nosenlen <- glm(sent_type ~ zeb_surprisal + zeb_mutual +postag_converted+ frequency_negative_ln_first_cs_word_trans + length_first_cs_word_trans, data=input_R_v2, family = "binomial")
lrtest(m_complete_zeb_both_nosenlen, m_complete_zeb_both)

# Both minus pos
m_complete_zeb_both_nopos <- glm(sent_type ~ zeb_surprisal + zeb_mutual + translation_sentence_length + frequency_negative_ln_first_cs_word_trans + length_first_cs_word_trans, data=input_R_v2, family = "binomial")
lrtest(m_complete_zeb_both_nopos, m_complete_zeb_both)
